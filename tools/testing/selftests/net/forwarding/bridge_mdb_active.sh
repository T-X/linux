...
mcast_active_check()
{
	...
	ip -d -j link show dev br0\
		| jq -e ".[] | select(.linkinfo.info_data.mcast_active_$af == $state)" &> /dev/null
...
mcast_vlan_active_check()
{
	...
	bridge -j vlan global show dev br0\
		| jq -e ".[].vlans.[] | select(.vlan == $vid and .mcast_active_$af == 1)" &> /dev/null
	ret="$?"
...
wait_lladdr_dad() {
	ip -6 -j a s dev "$1"\
		| jq -e ".[].addr_info | map(select(.scope == \"link\" and ((.tentative == true) | not))) | .[]" &> /dev/null
}
